...
export default function ProfilePage() {
  ...
  //   setMyProfile((prevData) => ({ ...prevData, ...updatedData }));
  //   setIsEditModalOpen(false);
  // };

  const fetchPointsHistory = async () => {
    const response = await getMyPointsHistories();
    const histories = response.data.pointsHistories.sort(
      (a: PointHistory, b: PointHistory) =>
        new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
    );
    return setMyPointsHistories(histories);
  };
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row gap-6 bg-white p-6 rounded-lg shadow-sm">
          <Avatar className="w-32 h-32 border-4 border-blue-500">
            ...
            />
            <AvatarFallback>{myProfile?.username}</AvatarFallback>
          </Avatar>
          <div className="flex-grow space-y-4">
            <div className="flex justify-between items-start">
              <div>
                <h1 className="text-3xl font-bold">{myProfile?.username}</h1>
                {/*<p className="text-xl text-gray-600">{myProfile.role}</p>*/}
              </div>
          ...
          <Card className="bg-white">
            <CardContent className="p-6">
              ...
              <div className="text-4xl font-bold text-blue-600">
                {myProfile?.money?.toLocaleString()}
              </div>
          ...
          <Card className="bg-white">
            <CardContent className="p-6">
              ...
              <div className="text-4xl font-bold text-yellow-600">
                {myProfile?.reliability}
              </div>
              <Progress
                value={myProfile?.reliability}
          ...
          <TabsContent value="points" className="mt-6">
            <Card className="bg-white border-gray-200 shadow-sm">
              <CardContent className="p-6">
                ...
                <div className="space-y-4">
                  {myPointsHistories?.map((item, index) => (
                    <div
                      key={index}
                      className="flex justify-between items-center"
                    >
                      <div>
                        <p className="font-medium">{item.contents}</p>
                        <p className="text-sm text-gray-600">
                          {new Date(item.createdAt).toLocaleString()}
                        </p>
                      </div>
                      <Badge
                        variant={item.amount < 0 ? 'destructive' : 'default'}
                        className={cn(
                          item.amount > 0 && 'bg-green-500 text-white',
                          item.amount < 0 && 'bg-red-500 text-white'
                        )}
                      >
                        {item.amount > 0 ? '+' : ''}
                        {item.amount}
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-lg relative">
            ...
            {/*    <Input id="name" name="name" defaultValue={myProfile.name} />*/}
            {/*  </div>*/}
            {/*  <div>*/}
            {/*    <Label htmlFor="role">직업</Label>*/}
            {/*    <Input id="role" name="role" defaultValue={myProfile.role} />*/}
            {/*  </div>*/}
            {/*  <div>*/}
            ...
            {/*    <Textarea id="bio" name="bio" defaultValue={myProfile.bio} />*/}
            {/*  </div>*/}
